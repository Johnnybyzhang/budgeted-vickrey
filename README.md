...
### Figure references (repository-relative)

> *These are references to existing or placeholder images generated by `src/budgeted_spa/plots.py`. Use them directly in your PDF/Canvas/Overleaf; paths assume `figures/` at repo root.*

1. **Price vs Round (mean ± 95% CI)** — `figures/price_path.png`
   *Caption:* Mean price by round with 95% confidence bands. Interpreted as revenue in second-price auctions. (Referenced in Theory §4 and Interpretation.)
2. **Budget Trajectories (median)** — `figures/budget_trajectories.png`
   *Caption:* Median budget across rounds; illustrates intertemporal conservation vs. spending. (Theory §5.)
3. **Revenue vs Round (mean ± 95% CI)** — `figures/revenue_by_round.png`
   *Caption:* Revenue path mirroring price; helpful for welfare/revenue trade-off discussion. (Theory §4.)
4. **Stage Heatmap (diagonal grid)** — `figures/stage_heatmap_eq_count.png` and/or `figures/stage_heatmap_truth_capped_br.png`
   *Caption:* Diagonal cases ($v_1=v_2$, $B_1=B_2$); color encodes equilibrium count or truthful-capped best-response metric. (Equilibrium & Refinements.)

> **Inline Markdown usage example:**
> `![Figure 1: Price vs Round](figures/price_path.png)`
